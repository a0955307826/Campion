...
$.each(uncompletedItineraries, function (index, row) {
	...
	<div class="row">
		...
		<div class="member_commentWriteItem">
			...
			<textarea data-row-index=${index} name="member_commentWrite" placeholder="請輸入行程心得，長度為1-150字"></textarea>
		</div>
...
$.ajax({
	...
	$(".member_editorLogo").attr("style", "background: url('./images/avatar/" + `${response[0]['PICTURE']}` + "'); background-repeat: no-repeat; background-size: cover; background-position: center;");
